...
const App: React.FC = () => {
      ...
      <div className="min-h-screen bg-silk flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white border border-morning shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] p-12 space-y-8 animate-fadeIn">
          <div className="text-center space-y-4">
            <div className="w-52 h-52 mx-auto flex items-center justify-center mb-6">
              <img src="/logo.svg" alt="Logo" className="w-full h-full object-cover object-left" />
            </div>
      ...
      <div className="min-h-screen bg-silk overflow-x-hidden">
        ...
        <nav className="h-24 px-12 flex items-center justify-between border-b border-morning bg-white/80 backdrop-blur sticky top-0 z-50">
          <div className="flex items-center space-x-3">
            <img src="/full-logo.svg" alt="ResearchCollab.ai" className="h-8 w-auto" />
          </div>
        ...
        {/* Hero Section */}
        {/* Hero Section */}
        <div className="w-full bg-[url('/herobg.png')] bg-cover bg-center bg-no-repeat">
          <div className="max-w-7xl mx-auto px-12 pt-32 pb-48 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <div className="space-y-12 animate-fadeIn">
              <div className="space-y-6">
                <span className="text-xs font-bold uppercase tracking-[0.4em] text-dodgeroll">Institutional Research Intelligence</span>
  ...
  return (
    <div className="flex h-screen bg-silk overflow-hidden font-sans">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-white/95 backdrop-blur-sm p-6 flex flex-col pt-20 animate-fadeIn lg:hidden">
          <div className="space-y-6">
            <nav className="flex flex-col space-y-4">
              {[
                ...
                <button
                  ...
                  className={`w-full p-6 flex items-center space-x-4 rounded-xl border group transition-all duration-200 ${view === item.view ? 'bg-hadfield text-white border-hadfield shadow-lg' : 'bg-white text-cerebral border-morning hover:border-hadfield'
                    }`}
                >
            ...
            <Button variant="ghost" size="full" onClick={() => setView(AppView.LANDING)}>Return Home</Button>
          </div>
        </div>
      )}

      {/* DESKTOP SIDEBAR (10%) - Hidden on Mobile */}
      <div className="hidden lg:flex w-[80px] shrink-0 border-r border-morning bg-white flex-col items-center py-10 space-y-10">
        <div className="w-10 h-10 flex items-center justify-center">
          <img src="/logo.svg" alt="R" className="w-full h-full object-contain" />
        </div>
        ...
        <nav className="flex-1 flex flex-col space-y-4">
          {[
            ...
            <button
              ...
              className={`w-14 h-14 rounded-xl flex flex-col items-center justify-center transition-all duration-200 group ${view === item.view ? 'bg-hadfield text-white shadow-lg' : 'text-cerebral hover:bg-silk'
                }`}
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white">
        ...
        <header className="h-16 lg:h-20 px-6 lg:px-10 border-b border-morning flex items-center justify-between shrink-0 bg-white z-20 relative">
          <div className="flex items-center space-x-4">
            {/* Mobile Hamburger */}
            <button className="lg:hidden text-gray-900 min-w-[48px] min-h-[48px] flex items-center justify-center -ml-3" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={isMobileMenuOpen ? "M6 18L18 6M6 6l12 12" : "M4 6h16M4 12h16M4 18h16"} /></svg>
            </button>

            <div className="space-y-0.5">
              <div className="flex items-center space-x-2">
                <span className="text-[8px] lg:text-[10px] uppercase font-bold tracking-widest text-hadfield">Workspace Active</span>
                <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" />
              </div>
              <h2 className="text-base lg:text-xl font-bold tracking-tight text-gray-900 truncate max-w-[200px] lg:max-w-md">{query || 'System Default Domain'}</h2>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="hidden lg:block w-px h-6 bg-morning" />
            <Button variant="primary" size="sm" className="hidden lg:flex">Export Report</Button>
          </div>
        </header>

        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          {/* MAIN WORKSPACE (60%) */}
          <section className="w-full lg:w-[60%] flex flex-col border-b lg:border-b-0 lg:border-r border-morning bg-silk/30 overflow-hidden relative">
                ...
                <>
                  {view === AppView.DASHBOARD && (
                    <div className="space-y-10 animate-fadeIn max-w-4xl mx-auto">
                      <div className="bg-white p-8 lg:p-12 border border-morning shadow-sm space-y-6">
                        <h3 className="text-xs uppercase font-bold tracking-[0.3em] text-hadfield">Intellectual Context</h3>
                        <p className="text-2xl lg:text-h2 font-bold tracking-tighter leading-tight text-gray-900">
                  ...
                  {view === AppView.MIND_MAP && (
                    <div className="h-[600px] lg:h-[750px] bg-white border border-morning shadow-2xl relative overflow-hidden animate-fadeIn">
                      <div className="absolute top-4 left-4 lg:top-8 lg:left-8 z-10 space-y-4">
                        ...
                        <div className="flex flex-col space-y-2 hidden lg:flex">
                      ...
                      <ResearchGraph interactive data={mindMapData} />
                      <div className="absolute bottom-8 right-8 z-10 p-6 bg-white/80 backdrop-blur border border-morning max-w-xs space-y-4 hidden lg:block">
                  ...
                  {view === AppView.OUTLINE && outlineData && (
                    <div className="max-w-4xl mx-auto bg-white border border-morning shadow-[0_48px_96px_-12px_rgba(0,0,0,0.05)] p-8 lg:p-24 space-y-20 animate-fadeIn min-h-screen">
                      <div className="text-center space-y-8 pb-12 border-b-4 border-morning">
                        ...
                        <h1 className="text-3xl lg:text-h1 font-bold tracking-tighter leading-none text-gray-900">{outlineData.title}</h1>
                        <div className="flex items-center justify-center space-x-4">
                          <div className="hidden lg:block w-8 h-px bg-morning" />
                          <span className="text-[8px] lg:text-[10px] font-bold uppercase tracking-widest text-cerebral">ResearchCollab Intelligence Architect</span>
                          <div className="hidden lg:block w-8 h-px bg-morning" />
                        </div>
                      </div>
                      ...
                      <div className="space-y-16 lg:space-y-24">
                        {outlineData.sections.map((section, idx) => (
                          <div key={idx} className="space-y-10">
                            <div className="flex flex-col lg:flex-row lg:items-baseline space-y-4 lg:space-y-0 lg:space-x-6">
                              <span className="text-4xl lg:text-5xl font-bold text-hadfield/20 font-mono tracking-tighter">{String(idx + 1).padStart(2, '0')}</span>
                              <h2 className="text-2xl lg:text-3xl font-bold tracking-tight text-gray-900 border-b border-silk pb-4 flex-1 uppercase tracking-tight">{section.heading}</h2>
                            </div>
                            <div className="lg:pl-[76px] space-y-8">
                              ...
                              <ul className="space-y-5">
                                {section.subheadings.map((sub, sidx) => (
                                  <li key={sidx} className="flex items-center space-x-4 group cursor-pointer">
                                    <div className="w-2 h-2 bg-hadfield opacity-20 group-hover:opacity-100 transition-opacity shrink-0" />
                                    <span className="text-base lg:text-lg font-medium text-gray-800 tracking-tight group-hover:text-hadfield transition-colors">{sub}</span>
                                  </li>
                                ))}
                              </ul>
                            </div>
                      ...
                      {/* Mobile Sticky Output Action */}
                      <div className="lg:hidden fixed bottom-6 left-6 right-6 z-40">
                        <Button variant="primary" size="full" className="shadow-2xl shadow-blue-900/40">Export Draft PDF</Button>
                      </div>
                    </div>
                  )}
                </>
            ...
            {/* Mobile Sticky Action Button for Dashboard */}
            {view === AppView.DASHBOARD && !loading && (
              <div className="lg:hidden fixed bottom-6 left-6 right-6 z-40">
                <div className="bg-white p-2 rounded-full shadow-2xl border border-morning flex items-center justify-between pl-6 cursor-pointer hover:bg-silk transition-colors" onClick={() => setView(AppView.MIND_MAP)}>
                  <span className="text-xs font-bold uppercase tracking-widest text-hadfield">View Topology Map</span>
                  <div className="w-10 h-10 bg-hadfield rounded-full flex items-center justify-center text-white">
          ...
          {/* CONTEXTUAL TOOLS (30%) - Elite level clarity - Hidden on Mobile */}
          <aside className="hidden lg:flex w-[30%] bg-white p-12 overflow-y-auto custom-scrollbar flex-col">
